...
它承接已批准的 design，重点回答：
- 先做哪一步
- 这一步的触碰范围（允许是高层级：模块 / 文档路径，不下沉到 file:line / 函数级）
- 这一步唯一容易分叉的判断点是什么
- 怎么验证这一步完成

plan 的意义不是“把 design 写得更长”，而是提供**可执行的分步计划**；具体改哪个文件由 implement 阶段决定。
...
- `cs-feat-plan/reference.md` 里的 Step 模板是 paradigm 维度的分步计划，不下沉到 file:line / 函数级；具体改哪个文件由 implement 阶段决定。
