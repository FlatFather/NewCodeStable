...
**触发时机**：用户触发 cs-feat 且**无已有产物**时，在进入路由表前先做分诊。

**目标**：识别需求类型，对"扩展现有功能"场景强制复用分析。
...
> 我理解你想做：{AI 一句话复述需求}。
> 
> 这是一个**全新的功能模块**，还是在**现有功能上扩展/优化**？

**判断信号**：
- 触发词："加"、"增加"、"优化"、"改进"、"扩展"、"给 XX 增加 YY 能力" → 偏向**扩展**
- 触发词："新建"、"创建"、"搭建"、"从零"、"新的 XX 系统" → 偏向**全新**
- 不确定：直接问用户

**不需要分诊的情况**（直接跳过）：
- 已有产物（design / plan / intent / brainstorm 任一存在）→ 按现有产物路由
- 用户明确说"brainstorm"或想法模糊 → 直接进 brainstorm
- 用户说"fastforward" → 直接进 fastforward
- 用户说"roadmap 里的 XX" → 按 roadmap 路由

### 路由规则

**情况 1：扩展现有功能**
```
用户确认是"扩展现有" → 告知"将强制复用分析" → cs-feat-design
                                              ↓
                        启动检查第 3 步会搜索相关代码并产出复用清单
                                              ↓
                              design 第 1.5 节基于清单设计扩展方案
```

**提示用户**（扩展场景）：
> 好的，这是扩展现有功能。我会在设计前先搜索相关代码，找出可以复用/扩展的部分，避免重复实现。

**情况 2：全新功能**
```
用户确认是"全新功能" → 正常流程 → cs-feat-design / brainstorm / fastforward
                                  ↓
                    启动检查第 3 步仍会搜索（防止用户误判）
```

**提示用户**（全新场景）：
> 好的，全新功能。我会在设计时仍会搜索相关代码，确保没有遗漏可复用的部分。
...
```
用户："给用户列表页加个搜索功能"
AI：  "我理解你想做：给用户列表页加搜索功能。
      这是全新的功能模块，还是在现有列表页上扩展？"
用户："扩展现有列表页"
AI：  "好的，扩展现有功能。我会先搜索用户列表页相关代码，
      找出可以复用的查询逻辑和 UI 组件。→ 进入 cs-feat-design"
```
...
用户："搭建一个权限管理系统"
AI：  "我理解你想做：搭建权限管理系统。
      这是全新的功能模块，还是在现有权限逻辑上扩展？"
用户："全新系统"
AI：  "好的，全新功能。我会在设计时仍会搜索相关代码，
      确保没有遗漏可复用的基础设施。→ 进入 cs-feat-design / brainstorm"
```
...
AI：  "我理解你想做：优化用户权限校验。
      这是全新的功能模块，还是在现有权限逻辑上扩展？"
用户："不太确定，现在有权限逻辑吗？"
AI：  "让我先搜索一下... {fast-context 搜索 '权限 / permission / auth'}
      → 发现现有 check_permission() 函数，建议扩展现有逻辑。
      → 进入 cs-feat-design（强制复用分析）"
```
