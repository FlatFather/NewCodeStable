...
5. **复用策略验证**（新增）：
   ...
   - 如果策略是"复用为主"，plan 的第一步必须是读取并确认 design 列出的直接复用点；后续步骤只能接入或配置既有能力，不得无理由新建等价实现
...
当 design 第 1.5 节策略是"复用为主"或"扩展为主"时，plan 的推进顺序必须体现既有实现优先。

**复用场景的 plan 模板**（design 1.5 节是"复用为主"）：
```markdown
## 2. 分步计划

### Step 1 — 确认直接复用边界
- **目标**：读取并确认 design 1.5 节列出的既有能力可直接承接本次需求
- **触碰范围**：{直接复用项所在文件与调用点}
- **退出信号**：已确认接口、输入输出、调用约束和现有测试覆盖；不需要复制或重写等价逻辑
- **验证**：阅读代码 + 跑相关现有测试

### Step 2 — 接入既有能力
- **目标**：通过已有公开接口、配置或组合方式接入能力
- **触碰范围**：{调用方 / 配置文件}
- **关键判断点**：不引入与复用项职责重叠的新模块或函数
- **退出信号**：需求路径复用既有实现并保持现有调用方行为
- **验证**：跑相关测试
...
**判断规则**：
- design 第 1.5 节策略 = "复用为主" → 使用复用场景模板
