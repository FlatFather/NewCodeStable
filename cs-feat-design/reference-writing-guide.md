...
**新增节**，建立"复用优先"文化。基于启动检查第 3 步的复用清单，明确本次策略。

**复用清单**（直接引用启动检查第 3 步的产出）：
```markdown
## 复用清单（来自启动检查）
...
- 扩展点列表：
  - 扩展点 1：函数 A.参数 X
    ...
    - 新增逻辑：一句话描述扩展的逻辑
  - 扩展点 2：模块 B.新增函数 C
    - 为什么放在 B 而不是新建模块？理由：XXX

如果选择"新增为主"：
- **必须逐项说明**：为什么不复用清单中的每个"可扩展后复用"项
- 新增理由格式：
  ```
  不扩展 [函数名] 的理由：
  - [ ] 职责不匹配（具体：XXX）
...
**写作要求**：
- 此节**必填**，不允许省略
- 如果启动检查第 3 步复用清单有"可扩展后复用"项，默认应选择"扩展为主"
- 选择"新增为主"时，每个"可扩展"项必须有显式理由
- 此节是 acceptance 阶段"复用策略审查"的对照依据
...
> 3. **第 1.5 节复用与扩展策略：复用清单是否完整？策略选择是否合理？如果选"新增为主"，理由是否充分？**
> 4. 第 2.1 名词层：现状描述对不对？变化是否覆盖所有数据/接口改动？
> 5. 第 2.2 编排层：主流程图和现状→变化能不能跑通你脑子里的场景？流程级约束有没有漏？
> 6. 第 2.3 挂载点：照这份清单能不能完整卸载？有没有项是内部改动被误列进来？
> 7. 第 2.5 结构健康度：评估的文件 + 目录是否准确？结论（不做 / 拆文件 / 重组目录）和方案是否同意？若有"建议沉淀的 convention"是否真属稳定模式？"超出范围的观察"有没有遗漏或写错？
