...
详细动作见 `initialization-mode.md`。本技能在该入口只建目录和草稿骨架，告知用户填完后再回来，**本轮结束，不继续推进 design 流程**。
...
落到挂载点清单（第 2.3 节）。**判据**：删掉这一项，feature 在用户/系统视角是不是就消失了？是→列，否→不列。详细 ✅/❌ 例子和写法看 `reference-writing-guide.md`。这清单顺带帮你发现自己有没有不小心往太多地方插桩——真挂入点越多代表耦合越散，是个信号。
...
3. **相关代码搜索与复用分析**（强制，必须产出复用清单）：
   - 从需求中提取 3-5 个核心关键词
   - 按 `reuse-analysis-guide.md` 执行代码搜索、分类判定和复用清单输出
   - 找到 ≥1 个相关函数时，必须判断是“可直接复用”/“可扩展后复用”/“不可复用”
   - 有“可扩展后复用”项时，必须在 design 第 1.5 节优先考虑扩展方案；选择新增时写明不扩展理由

4. **读需求相关的现有代码**——基于第 3 步的复用清单，深入读取相关代码文件

**按信号触发 3 条**（没信号跳过）：

- **复用机会强提醒**——如果第 3 步复用清单有"可扩展后复用"项，在进入第 2 步前必须向用户明确提示："发现可扩展的现有函数 [X]，建议优先设计扩展方案。如果最终选择新增，需在 design 第 1.5 节说明理由。"
...
### 2. 想清楚这功能该放在哪儿（基于启动检查第 3 步的复用清单）

动笔写名词层 / 编排层前，**结合第 1 步的复用清单**，回答：**这次要加的东西在项目整体结构里属于哪儿？**

**决策优先级（复用优先文化）**：

**优先级 P0：扩展现有模块**
- 第 3 步复用清单有"可扩展后复用"项？→ **优先在该函数/模块所在位置扩展**
- 判断信号：
  - 同一张表/同一个 API 的查询 → 合并查询逻辑
  - 相似的业务逻辑 → 扩展现有函数（加参数/加分支）
  - 职责相近的模块 → 在该模块内新增函数
- 设计要点：
  - 扩展点在哪？（函数参数/返回值/内部分支）
  - 如何保证向后兼容？（可选参数/版本控制）
...
**优先级 P1：复用现有模块结构**
- 复用清单标记"不可复用"但找到了相关模块 → 在该模块内新增函数（而非新建模块）
- 判断信号：项目已有处理类似职责的模块
- 理由：保持职责内聚，避免模块碎片化
...
- 仅当以下情况才新建：
  - 复用清单搜索结果为空（无相关代码）
  - 职责完全不同，强行放入现有模块会破坏内聚性
  - 技术栈/依赖不兼容
- **必须在 design 第 1.5 节写明**：为什么不能扩展复用清单中的每一项

**决策检查清单**：
- [ ] 复用清单有"可扩展"项？是 → 为什么不扩展？必须有明确理由
- [ ] 现有模块本该承担？是 → 在那个模块里扩展
- [ ] 横跨多个模块？→ 抽公共层 vs 让某一方主导
- [ ] 跟现有任何模块都不像？→ 确认复用清单搜索充分后，才新建

**代价提醒**：
- ❌ 忽略复用机会 → 重复造轮子，同一张表查两次
- ❌ 放错模块 → 职责混乱，"什么都装的筐"
- ❌ 新建平行实现 → 几个版本同存，维护成本高

**结论写进 design**：
- 第 1.5 节：复用与扩展策略（基于复用清单的决策）
- 第 1 节：决策与约束（模块归属）
- 第 4 节：架构影响（如果涉及新建模块或跨模块接口）
...
按 `reference-writing-guide.md` 写第 2 节四个子节（2.1 名词层 / 2.2 编排层 / 2.3 挂载点 / 2.4 推进策略）。重点提示：
...
- **第 2.5 节"结构健康度与微重构"是固定步骤**——按 `reference-writing-guide.md` 写作要求评估**两类对象**：要改的文件（文件级）+ 要落新文件的目标目录（目录级）。**评估前先查 compound 已有 convention**（关键词围绕"目录组织 / 文件归属 / 命名约定"），命中就直接照办。结论三选一：
...
按 `reference-writing-guide.md` 补齐剩余节（第 0 / 3 / 4 节）。初稿 frontmatter `status: draft`。
...
发一次整体 review 提示（提示词在 `reference-writing-guide.md` 第 5 节）。用户提意见就改，反复直到放行，把 `status` 从 `draft` 改 `approved`。
