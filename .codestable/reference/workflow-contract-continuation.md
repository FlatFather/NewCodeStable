...
当用户对当前阶段的必需 checkpoint 作出明确批准时，该回复同时完成两件事：

1. 将批准结果写入当前阶段的 canonical artifact；
2. 若下一阶段唯一、前提完整且无 blocker，立即 handoff 到下一技能。

不得在写入批准状态后，再要求用户补一次“继续 / 现在开始吗”才能进入确定性的下一阶段。只有出现新 scope、canonical 冲突、多候选或新的独立决策点时，才允许再次停下询问。
