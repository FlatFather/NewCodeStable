...
- `cs-feat` — 新功能,design → plan → implement → acceptance；`cs-feat-design` 只负责范围与约束，`cs-feat-plan` 负责生成 `plan.md` + `checklist.yaml` 并形成进入实现前的独立确认关口（想法还模糊时先走讨论层 `cs-brainstorm` 做分诊，不属于 feature 流程内部）。fastforward 保持为独立快路径。
