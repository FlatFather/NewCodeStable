...
- `cs-feat` — 新功能,design → implement → acceptance；对复杂 feature 可走 hybrid 口径：design → plan → checklist 驱动实现 → acceptance（想法还模糊时先走讨论层 `cs-brainstorm` 做分诊，不属于 feature 流程内部）。一旦进入 hybrid 口径，`plan.md` 就是必备产物。
...
- **单次动作**(feature / issue / refactor)——本次要做的一件具体事情的 spec。feature 目录允许两种合法口径：legacy（design + checklist + acceptance）和 hybrid（design + plan + checklist + acceptance）。动作走完后,相关沉淀提炼进愿景档案、结构档案和沉淀类文档
...
feature 走 brainstorm(可选) → design → implement → acceptance；复杂 feature 可在已批准 design 之后增加 `plan` 作为详细步骤正文，issue 走 report → analyze → fix。每个阶段有退出条件,上一个没满足,下一个不开始。
