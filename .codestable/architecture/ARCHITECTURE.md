...
- **feature-plan**：hybrid feature 的详细执行步骤正文，承接已批准 design，只展开推进顺序、退出信号、验证路径与风险缓解。
...
- **feature 流程**：默认主线是 `design → checklist → implement → acceptance`；复杂 feature 可以在 design 之后引入 `plan` 层，形成 `design → plan → checklist → implement → acceptance` 的 hybrid 流程。
