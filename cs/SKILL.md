...
- **continuation-first 命中唯一 fresh、无 blocker 候选时**：直接说明“检测到你在继续 {对象}，现在进入 `{cs-xxx}` 的 {阶段}。”然后进入对应子技能；**不得**再问“现在切到吗？”
- **只用于非 continuation 的普通路由，且诉求可唯一分诊时**：说明“你这个诉求走 `{cs-xxx}`——{一句话理由}，现在进入该技能。”并直接进入；不要把路由结论再包装成确认题。
- **只有路由本身不唯一、会改变用户表达的目标、或命中安全 gate 时**，才列出缺口并询问用户。
